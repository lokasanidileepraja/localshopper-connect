...
const HomePage = () => {
  ...
  return (
    <div className="bg-background min-h-screen pb-4">
      ...
      <motion.section {...fade(0.08)} className="px-4">
        <div className="grid grid-cols-6 gap-0">
          {CATS.map((cat) => (
            ...
            >
              <div className="w-13 h-13 rounded-full bg-muted/70 flex items-center justify-center text-[24px]">
                {cat.emoji}
              </div>
      ...
      {/* ━━━ Trust ━━━ */}
      <motion.section {...fade(0.15)} className="px-4 py-3">
        <div className="flex gap-2">
          {[
            ...
            <div key={t.text} className="flex-1 flex items-center gap-2 py-3 px-2.5 rounded-xl bg-muted/40">
              <t.icon className="h-4 w-4 text-primary shrink-0" />
              <span className="text-[11px] font-medium text-muted-foreground leading-tight">{t.text}</span>
            </div>
          ))}
        </div>
      </motion.section>
...
const Section = ({
  ...
  <section className="mt-6">
    <div className="flex items-baseline justify-between px-4 mb-3">
      ...
      {action && onAction && (
        <button onClick={onAction} className="text-xs font-semibold text-primary flex items-center gap-0.5">
